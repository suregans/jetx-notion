...
function jetx_hub_admin_notices(): void {
	$notices = [
		...
		'detected'  => [ 'success', 'Notion fields detected and saved.' ],
		'det_error' => [ 'error',   'Field detection failed — check your token and database ID on the Connection tab.' ],
	];
	...
	foreach ( $notices as $key => $config ) {
		if ( isset( $_GET[ $key ] ) ) {
			$type = $config[0];
			$msg  = $config[1];
			// For det_error pass the actual error message if present.
			if ( $key === 'det_error' && ! empty( $_GET['msg'] ) ) {
				$msg = 'Field detection failed: ' . sanitize_text_field( urldecode( $_GET['msg'] ) );
			}
